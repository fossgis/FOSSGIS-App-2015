...
function showMyEvents () {
    ...
    $.ajax({
      ...
      obj.forEach(function (speech) {
        ...
        $("#myevent").append("<div id='infos"+speech.number+"' class='reveal-modal' data-reveal><h2>"+speech.title+"</h2><p class='lead'>Raum: "+speech.room+"</p><p>Dauer: "+speech.duration+"</p><p>"+speech.description+"</p><iframe width='100%' height='200px' src='http://pb.fossgis.de/feedback/FOSSGIS2014/event/741.de.html'></iframe><a class='close-reveal-modal'>&#215;</a></div>");
      });
    }).fail(function (err) {
      console.log(err);
    }).always(function () {
    ...
    if (search.length > 3) {
      $.ajax({
        ...
        obj.forEach(function (speech) {
          ...
          $("#mysearch").append("<div id='infos"+speech.number+"' class='reveal-modal' data-reveal><h2>"+speech.title+"</h2><p class='lead'>"+speech.subtitle+"</p><p>Raum: "+speech.name+"</p><p>Dauer: "+speech.duration+"</p><p>Referent: "+speech.speaker+"</p><p>"+speech.description+"</p><a class='close-reveal-modal'>&#215;</a></div>");
        });
      }).fail(function (err) {
        console.log(err);
      }).always(function () {
